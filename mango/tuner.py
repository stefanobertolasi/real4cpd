...
class Tuner:
    ...
    def run(self, verbose=False):
        if self.config.is_bayesian:
            self.results = self.runBayesianOptimizer(verbose=verbose)
    ...
    def runBayesianOptimizer(self, verbose=False):
        results = dict()
        ...
        calls = range(self.config.num_iteration)
        if verbose:
            calls = tqdm(range(self.config.num_iteration))
        ...
        for _ in calls:
            ...
            if verbose:
                calls.set_description("Best loss: %s" % results["best_objective"])
        ...
        # print(f'Best threshold related to best score:   {results['best_threshold']}')
        # print(f'Best weights related to best score:   {results['best_params']}')
