...
class BayesianLearning(BasePredictor):
    ...
    def remove_duplicates_MetaTuner(self, X, X_Sample, mu, Value, Value_ext):
        ...
        return (
            X[v_sorting_index[index]],
            Value[v_sorting_index[index]],
            Value_ext[v_sorting_index[index]],
            mu[v_sorting_index[index]],
        )
    ...
    def get_next_batch_MetaTuner(
        ...
        for i in range(batch_size):
            ...
            X_next, s_value, s_value_ext, u_value = (
                self.Upper_Confidence_Bound_Remove_Duplicates_MetaTuner(
                    X_tries, X_temp, batch_size, exploration_factor, Optimizer_iteration
                )
            )
